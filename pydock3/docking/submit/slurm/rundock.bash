...
log SLURM_ARRAY_JOB_ID=SLURM_ARRAY_JOB_ID
log SLURM_ARRAY_TASK_ID=$SLURM_ARRAY_TASK_ID
log df=$(df)


# validate required environmental variables
for var in EXPORT_DEST INPUT_SOURCE DOCKFILES DOCKEXEC SHRTCACHE LONGCACHE SLURM_ARRAY_JOB_ID SLURM_ARRAY_TASK_ID; do
  if [[ -z var ]]; then
    echo "One or more of the following require environmental variables are not defined: EXPORT_DEST INPUT_SOURCE DOCKFILES DOCKEXEC SHRTCACHE LONGCACHE SLURM_ARRAY_JOB_ID SLURM_ARRAY_TASK_ID"
    exit 1
  fi
done

# initialize all our important variables & directories
JOB_DIR=${SHRTCACHE}/$(whoami)/${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}
INPUT_TARBALL=$(sed "${SLURM_ARRAY_TASK_ID}q;d" $EXPORT_DEST/joblist | awk '{print $1}')
...
#
OUTPUT=${EXPORT_DEST}/$(sed "${SLURM_ARRAY_TASK_ID}q;d" $EXPORT_DEST/joblist | awk '{print $2}')
LOG_OUT=${SHRTCACHE}/rundock_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.out
LOG_ERR=${SHRTCACHE}/rundock_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.err
