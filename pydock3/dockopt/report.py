...
def generate_dockopt_job_report(
    dockopt_job_dir_path=".",
    pdf_path="dockopt_job_report.pdf",
    opt_results_csv_file_name="dockopt_job_results.csv",
    enrichment_metric="enrichment_score",
):
    opt_results_csv_file_path = os.path.join(
        dockopt_job_dir_path, opt_results_csv_file_name
    )
    ...
    with PdfPages(pdf_path) as f:
        ...
        image_file_path = os.path.join(
            dockopt_job_dir_path, "best_retrodock_job", "roc.png"
        )
        ...
        fig = plt.figure(figsize=(11.0, 8.5))
        image_file_path = os.path.join(
            dockopt_job_dir_path, "best_retrodock_job", "energy.png"
        )
        image = mpimg.imread(image_file_path)
        plt.axis("off")
        ...
        fig = plt.figure(figsize=(11.0, 8.5))
        image_file_path = os.path.join(
            dockopt_job_dir_path, "best_retrodock_job", "charge.png"
        )
        image = mpimg.imread(image_file_path)
        plt.axis("off")
        ...
        multivalued_config_param_columns = [
            ...
            if column not in POSSIBLE_NON_PARAMETER_COLUMNS and df[column].nunique() > 1
        ]
