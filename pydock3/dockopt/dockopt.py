...
class AbstractTargetsDAG(object):
    def __init__(self, config, blaster_files, working_dir):
        ...
        for step in steps:
            ...
            for infile in step.infiles:
                g.add_node(
                    ...
                    target=infile.original_file_in_working_dir,
                )
            ...
            for outfile in step.outfiles:
                g.add_node(
                    ...
                    target=outfile.original_file_in_working_dir,
                )
            ...
            for (infile_step_var_name, infile), (
                ...
                outfile,
            ) in itertools.product(infiles_dict_items_list, outfiles_dict_items_list):
                g.add_edge(
                    infile.original_file_in_working_dir.name,
                    outfile.original_file_in_working_dir.name,
                    step=step,
        ...
        logger.debug(
            f"AbstractTargetsDAG initialized with:\n\tNodes: {self.g.nodes}\n\tEdges: {self.g.edges}"
        )
...
class FullTargetsDAG(object):
    def __init__(self, abstract_targets_dag, config, working_dir):
        ...
        def process_abstract_node(abstract_node, abstract_targets_dag):
            ...
            if abstract_targets_dag.g.in_degree(abstract_node) > 0:
                ...
                unprocessed_predecessors = [
                    pred
                    for pred in predecessors
                    if pred not in abstract_nodes_processed
                ]
                for (
                    unprocessed_pred
                ) in (
                    unprocessed_predecessors
                ):  # all predecessor nodes need to be processed first
            ...
            if "target" in abstract_node_data_dict:
                ...
                if abstract_targets_dag.g.in_degree(abstract_node) > 0:
                    abstract_predecessors = list(
                        abstract_targets_dag.g.predecessors(abstract_node)
                    ...
                    for abstract_pred in abstract_predecessors:
                        abstract_pred_node_to_corresponding_full_dag_pred_nodes_dict[
                            ...
                            full_dag_pred_node
                            for full_dag_pred_node, data in self.g.nodes(data=True)
                            if data.get("abstract_node_name") == abstract_pred
                        ]
                    items = list(
                        abstract_pred_node_to_corresponding_full_dag_pred_nodes_dict.items()
                    )
                    abstract_pred_nodes = [x[0] for x in items]
                    full_dag_pred_nodes_lists = [x[1] for x in items]
                    for i, full_dag_pred_nodes in enumerate(
                        itertools.product(*full_dag_pred_nodes_lists)
                    ):
                        ...
                        for full_dag_pred_node in full_dag_pred_nodes:
                            ...
                            full_dag_ancestors += list(
                                nx.ancestors(self.g, full_dag_pred_node)
                            )
                        full_dag_ancestors_abstract_node_names = [
                            self.g.nodes[full_dag_ancestor]["abstract_node_name"]
                            for full_dag_ancestor in full_dag_ancestors
                        ]
                        if len(list(set(full_dag_ancestors))) != len(
                            list(set(full_dag_ancestors_abstract_node_names))
                        ):
                            continue
                        ...
                        self.g.add_node(
                            ...
                            target=full_dag_target,
                            abstract_node_name=abstract_node,
                        )
                        for j, full_dag_pred_node in enumerate(full_dag_pred_nodes):
                            abstract_pred_node = abstract_pred_nodes[j]
                            abstract_edge = abstract_targets_dag.g.get_edge_data(
                                abstract_pred_node, abstract_node
                            )
                            self.g.add_edge(
                                full_dag_pred_node,
                                full_dag_target_name,
                                ...
                                parent_node_step_var_name=abstract_edge[
                                    "parent_node_step_var_name"
                                ],
                                child_node_step_var_name=abstract_edge[
                                    "child_node_step_var_name"
                                ],
                            )
                ...
                values = list(
                    set(
                        [
                            job_param_dict[abstract_parameter.name].value
                ...
                raise Exception(
                    f"Abstract targets DAG node found that is neither a parameter nor target: {abstract_node}"
                )
        ...
        for u, v, data in self.g.edges(data=True):
            ...
            step_identity_to_step_class_dict[data["step_identity"]] = data[
                "step_class"
            ]  # TODO: fix this lazy implementation

        for step_identity, edges in step_identity_to_edges_dict.items():
            kwargs = {
                "step_dir": Dir(path=os.path.join(working_dir.path, step_identity))
            }
        ...
        def get_ancestor_parameters(nodes):
            ...
            #
            ancestor_parameters = [
                self.g.nodes[ancestor_parameter_node]["parameter"]
        ...
        #
        abstract_dag_dock_file_node_name_to_dock_file_field_name_dict = {
            getattr(
                abstract_targets_dag.dock_files, dock_file_field.name
            ).name: dock_file_field.name
            for dock_file_field in fields(abstract_targets_dag.dock_files)
        }
        abstract_dag_dock_file_node_name_to_full_dag_dock_file_nodes_dict = {
            abstract_dag_dock_file_node_name: [
                full_dag_node
                for full_dag_node in self.g.nodes
                if self.g.nodes[full_dag_node].get("abstract_node_name")
                == abstract_dag_dock_file_node_name
            ]
            for abstract_dag_dock_file_node_name in abstract_dag_dock_file_node_name_to_dock_file_field_name_dict
        }
        abstract_dag_dock_file_node_names, full_dag_dock_file_nodes_lists = zip(
            *abstract_dag_dock_file_node_name_to_full_dag_dock_file_nodes_dict.items()
        )
        dock_files_combinations = []
        input_parameters_combinations = []
        for full_dag_dock_file_nodes in itertools.product(
            *full_dag_dock_file_nodes_lists
        ):
            if not self.nodes_have_consistent_lineages(
                full_dag_dock_file_nodes
            ):  # skip dockfile node combinations that have inconsistent lineages
                continue
            kwargs = {
                abstract_dag_dock_file_node_name_to_dock_file_field_name_dict[
                    abstract_dag_dock_file_node_name
                ]: self.g.nodes[full_dag_dock_file_node]["target"]
                for abstract_dag_dock_file_node_name, full_dag_dock_file_node in zip(
                    abstract_dag_dock_file_node_names, full_dag_dock_file_nodes
                )
            }
    ...
    def nodes_have_consistent_lineages(self, nodes):
        edges_in_lineages = list(
            set([edge for node in nodes for edge in self.get_edges_in_lineage(node)])
        )
        return len(
            list(
                set([self.g.edges[edge]["step_identity"] for edge in edges_in_lineages])
            )
        ) == len(
...
class Dockopt(Script):
    ...
    def init(self, job_dir_path=JOB_DIR_NAME, overwrite=False):
        ...
        retrodock_jobs_dir = Dir(
            path=os.path.join(job_dir.path, self.RETRODOCK_JOBS_DIR_NAME),
            create=True,
            reset=False,
        )
    ...
    def run(
        ...
        abstract_targets_dag = AbstractTargetsDAG(
            config=config, blaster_files=blaster_files, working_dir=working_dir
        )
        full_targets_dag = FullTargetsDAG(
            abstract_targets_dag=abstract_targets_dag,
            config=config,
            working_dir=working_dir,
        )
        logger.info("done.")
        ...
        if config.param_dict["matching_spheres_perturbation.use"].value:
            ...
            for node_name, node_data in full_targets_dag.g.nodes(data=True):
                if (
                    node_data.get("abstract_node_name")
                    == blaster_files.matching_spheres_file.name
                ):
                    spheres = read_sph(
                        os.path.join(working_dir.path, node_name),
                        chosen_cluster="A",
                        color="A",
                    )
                    ...
                    for i in range(
                        ...
                        perturbed_file_path = os.path.join(
                            working_dir.path, perturbed_file_name
                        )
                        unperturbed_file_name_to_perturbed_file_names_dict[
                            node_name
                            ...
                            perturbation_xyz = tuple(
                                [
                                    random.gauss(
                                        ...
                                        float(
                                            config.param_dict[
                                                "matching_spheres_perturbation.standard_deviation_angstroms"
                                            ].value
                                        ),
                                    )
                                    for _ in range(3)
            ...
            for i, dock_files_combination in enumerate(
                full_targets_dag.dock_files_combinations
            ):
                for (
                    perturbed_file_name
                    ...
                    new_dock_files_combination.matching_spheres_file = BlasterFile(
                        os.path.join(working_dir.path, perturbed_file_name)
                    )
                    dock_files_combinations_for_retro_docking.append(
                        new_dock_files_combination
                    )
                    input_parameters_combinations_for_retro_docking.append(
                        full_targets_dag.input_parameters_combinations[i]
                    )
        else:
            dock_files_combinations_for_retro_docking = (
                full_targets_dag.dock_files_combinations
            ...
            input_parameters_combinations_for_retro_docking = (
                full_targets_dag.input_parameters_combinations
            )

        #
        matching_spheres_perturbation_param_dict = {
            key: value
            for key, value in config.param_dict.items()
        ...
        for i, (
            ...
            indock_file = IndockFile(
                path=os.path.join(working_dir.path, indock_file_name)
            )
        ...
        for i, (dock_executable_path, dock_files, indock_file) in enumerate(
            ...
            retrodock_job_dir = Dir(
                path=os.path.join(retrodock_jobs_dir.path, retro_dock_job_num),
                create=True,
            )
        ...
        optimization_results_csv_file_path = os.path.join(
            job_dir.path, "dockopt_job_results.csv"
        )
        logger.debug(
        ...
        shutil.copytree(
            os.path.join(retrodock_jobs_dir.path, best_retrodock_job_num),
            best_retrodock_job_dir.path,
        )

        # copy docking configuration files to best job dir
        best_retrodock_job_dockfiles_dir = Dir(
        ...
        for file_name in retrodock_job_num_to_docking_configuration_file_names_dict[
            ...
            best_retrodock_job_dockfiles_dir.copy_in_file(
                os.path.join(working_dir.path, file_name)
            )
