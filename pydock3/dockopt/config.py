...
class DockoptParametersConfiguration(ParametersConfiguration):
    def __init__(self, config_file_path):
        ...
        # if value type is list then we treat key as optimizable
        # (i.e. it is a list of possible values to try)
        self.optimizable_parameter_keys = [
            key
            for key, param in self.param_dict.items()
            if isinstance(param.value, list)
        ]

        self.job_param_dicts = (
            self.get_param_dicts_for_different_optimization_configurations(
                self.param_dict
            )
        )
    ...
    def get_param_dicts_for_different_optimization_configurations(self, opt_param_dict):
        param_dicts = []
        opt_param_values_list = [
            opt_param_dict[opt_param_key].value
            for opt_param_key in self.optimizable_parameter_keys
        ]
        for opt_param_value_combination_tuple in itertools.product(
            *opt_param_values_list
        ):
            ...
            for i, opt_param_value in enumerate(opt_param_value_combination_tuple):
                ...
                param_dict[opt_param_key] = Parameter(
                    name=opt_param_key, value=opt_param_value
                )
