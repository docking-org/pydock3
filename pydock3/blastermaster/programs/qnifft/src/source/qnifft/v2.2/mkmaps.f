	subroutine mkmaps(natom,exrad,radprb)
c
...
c------------------------------------------------------------------
	...
	logical   it1(0:6),ihs
	dimension iline(65)
	integer nepsatm(0:100) ! debug kas 13-feb-01
c
...
c
c reset iepsmp using these boundary elements
c
c
	ihs = .true. ! debug kas 13-feb-01
	if(ihs) then
...
c debug
	...
	do k = 2,igrid-1
	  do j = 2,igrid-1
	    do i = 2,igrid-1
		do k1 = 1,3
		  if(iepsmp(i,j,k,k1).eq.1)then
		    iat = iepsmp2(i,j,k,k1)
		    nepsatm(iat) = nepsatm(iat) + 1
		  end if
		end do
          end do   
        end do      
	end do
	print *,'in mkmaps iat, nepsatm if iepsmp=1'
	ntot = 0
	do i = 0,natom
	  ntot = ntot + nepsatm(i)
	  print *,i,nepsatm(i)
...
c	print *,' '
	...
	integer*2 iepsmp2(ngrid,ngrid,ngrid,3)
	integer*1 debmap(ngrid,ngrid,ngrid) ! kas 8-feb-01
...
c
	...
	rad2a = rad2 - 0.25
		...
		if(distsq.lt.radp2)  debmap(ix,iy,iz) = 0 ! kas 8-feb-01
...
c
	...
	print *,'in setin iat, nepsatm if iepsmp2 > 0'
	ntot = 0
	do i = 0,natom
	  ntot = ntot + nepsatm(i)
	  print *,i,nepsatm(i)
