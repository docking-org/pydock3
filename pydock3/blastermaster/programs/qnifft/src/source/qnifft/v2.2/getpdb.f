...
c------------------------------------------------------------------
	...
	character*24 crdstr
	character*14 radstr
	logical isite,isitecrg,iatout,iatin
	character*80 pdbfili,pdbfilo,pdb2fili
        logical run_extra_pdb
c------------------------------------------------------------------
	dimension xo(3),attemp(3)
	dimension xn(3)
...
c-----------------------------------------------------------------
	...
	if(iatin)then
	  write(6,*)'using values in *.atm file'
	else
	  write(6,*)'using *.siz/*.crg files'
	end if
	write(6,*)'   '
	natom = 0.
	qnet = 0.
	...
	if(iatout) then
	  ...
	  print *,pdbfilo
	  write(6,*)'   '
	  write(19,*)'GRASP PDB FILE'
	  write(19,*)'FORMAT NUMBER= 1'
...
c
	  ...
	  crdstr = line(31:54)
        read(crdstr,205)attemp
	  if(.not.isite)then
	    do k = 1,3
	      atcrd(k,natom) = attemp(k)
	    end do
	  end if
	  if(iatin)then
	    radstr = line(55:68)
          read(radstr,206)radtmp,atcrg(natom)
	    if(.not.isite)atrad(natom)=radtmp
	    chrgv = atcrg(natom)
	  else
	    atm = line(12:17)
	    res = line(18:20)
...
c
	    ...
	    atcrg(natom) = chrgv
	  end if ! end of iatin branch
	  qnet = qnet + chrgv
	  if(chrgv.gt.0.0) then
	    qplus = qplus + chrgv
	    do k = 1,3
	      cqplus(k) = cqplus(k) + attemp(k)*chrgv
	    end do
	  end if
	  if(chrgv.lt.0.0) then
	    qmin = qmin + chrgv
	    do k = 1,3
	      cqmin(k) = cqmin(k) + attemp(k)*chrgv
	    end do
	  end if
c
...
c	end of file
	close(13)
        if (run_extra_pdb) then !running probe pdb file as well.
   	  open(13,file=pdb2fili,err=903,status='old')
	  print *,'reading additional coordinates from file'
	  print *,pdb2fili
          run_extra_pdb = .false. !don't read it more than once
          goto 103 !horrible, but this is the best way without a serious
             !coding spree to rewrite this part
	end if
	if(iatout) then
	  close(19)
	end if
