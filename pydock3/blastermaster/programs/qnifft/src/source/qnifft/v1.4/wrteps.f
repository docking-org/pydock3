      subroutine wrteps(epsfil,debfct,debfcti,debfcto)
c reformats epsilon array epsmap to compact array 
c full format: epsmap(65,65,65,3) real*4 bit format: neps(5,65,65,3) integer*2
...
9004		continue
            do 9008 ix=1,ngrid
              if(epsmap(ix,iy,iz,idir).eq.1.) then
c set bit
...
9014		continue
            do 9018 ix=1,ngrid
c debug
c              if(debmap(ix,iy,iz).ne.0.) then
              if(debmap(ix,iy,iz).eq.debfcti) then
c set bit
		   nout = nout + 1
