	...
	if(donon)then
     ...
     &        - phimap(ix,iy,iz+1)*epsmap(ix,iy,iz,3)
	        if(debmap(ix,iy,iz).ne.0.) then
	          ...
	          pold= min(pold,15.)  
		    fmap(ix,iy,iz) = fmap(ix,iy,iz) - debmap(ix,iy,iz)*
     &			 (cion(1)*zion(1)*exp(-zion(1)*pold)
     &			+ cion(2)*zion(2)*exp(-zion(2)*pold)
     &			+ cion(3)*zion(3)*exp(-zion(3)*pold)
     &			+ cion(4)*zion(4)*exp(-zion(4)*pold))
	        end if
...
c	print *,'zion: ',zion
	if(donon)then
     ...
     &			 (cion(1)*(zion(1)**2)*exp(-zion(1)*temp)
     &			+ cion(2)*(zion(2)**2)*exp(-zion(2)*temp)
     &			+ cion(3)*(zion(3)**2)*exp(-zion(3)*temp)
     &			+ cion(4)*(zion(4)**2)*exp(-zion(4)*temp))
              end if
	      end do
	    end do
	  end do
