! minimizer for one saved pose
      subroutine run_minimizer(options0, db2lig, ligscore,
...
c        copy the original rotation matrix to match_mini
         do i = 1, 3
              ...
              enddo
              do k = 1, 4
                  match_mini%xor(i,k,count1) = match%xor(i,k,matchnum)
                  match_mini%xos(i,k,count1) = match%xos(i,k,matchnum)
              enddo
...
c     &            setds,
     ...
     &            allminlimits, allmaxlimits, total_iterations)
          ...
          call get_time(time3)
          ! set score components in ligscore
          !ligscore_mini%pose_es(count1) = setes
          !ligscore_mini%pose_gi(count1) = setgi
          !ligscore_mini%pose_vs(count1) = setvs
          !ligscore_mini%pose_ps(count1) = setps
          !ligscore_mini%pose_as(count1) = setas
          !ligscore_mini%pose_is(count1) = db2lig%set_energy(setcount)
          !ligscore_mini%pose_rs(count1) = setrs
          !ligscore_mini%pose_ds(count1) = setds
          !ligscore_mini%pose_hs(count1) = seths
          !ligscore_mini%pose_score(count1) = setscore
     ...
     &        setas, 
     &        setps, 
c     &        db2lig%set_energy(setcount), 
c     &        setrs, 
     &        db2lig%set_total_strain(setcount), 
     &        db2lig%set_max_strain(setcount), 
     &        setds, 
