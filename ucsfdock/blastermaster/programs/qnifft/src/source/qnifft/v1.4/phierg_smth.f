...
c
	...
	do k = 2,igrid-1
	  do j = 2,igrid-1
	    do i = 2,igrid-1
c	      iin = (iepsmp(i,j,k,1) + iepsmp(i,j,k,2) 
c     &      + iepsmp(i,j,k,3) + iepsmp(i-1,j,k,1)
c     &      + iepsmp(i,j-1,k,2) + iepsmp(i,j,k-1,3))
c	      if((iin.ne.0).and.(iin.ne.6)) then
c kas 1 aug- new condition for smoothed dieletric
	      isame = 1
	      if(epsmap(i,j,k,1).ne.epsmap(i,j,k,2))isame=0
	      if(epsmap(i,j,k,1).ne.epsmap(i,j,k,3))isame=0
	      if(epsmap(i,j,k,1).ne.epsmap(i-1,j,k,1))isame=0
	      if(epsmap(i,j,k,1).ne.epsmap(i,j-1,k,2))isame=0
	      if(epsmap(i,j,k,1).ne.epsmap(i,j,k-1,3))isame=0
	      if(isame.eq.0)then
		isrf=isrf+1
	...
	if(iconc)then
	  ...
	  do k = 1,igrid
	    do j = 1,igrid
	      do i = 1,igrid
	        if(debmap(i,j,k).gt.0.)then
		    cnet = -rionst*2.*sinh(phimap(i,j,k))
		  else
		    cnet = 0.
	        end if
		  phimap(i,j,k) = cnet
