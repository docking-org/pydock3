...
c	open(lunit,file='qnifft_sas.usr',status='unknown')
...
c	if(npoint.gt.ngirdmx)then
	  print *,'WARNING: increase size of girdle point list from, to: ',ngirdmx,ngird
c	  print *,'WARNING: increase size of girdle point list from, to: ',ngirdmx,npoint
	  ngird = ngirdmx
c	  npoint = ngirdmx
	end if
	do n = 1,ngird
...
c		    print *,'after searching ',nlook
		  ...
		  if((ipt.ne.0))then
c		  if((ipt.ne.0).and.(dist.lt.0.5))then
...
c		      ncont1 = ncont1 + 1
c                  dmin2 = (atcrd(1,iat)-i)**2 + (atcrd(2,iat)-j)**2 + (atcrd(3,iat)-k)**2
c                  if(dmin2.gt.1.e-6)then
c                    srat = satrad(iat)/sqrt(dmin2)
c			  dist = sqrt(dmin2) - satrad(iat)
ccc
cc                   srat = 1. ! debug
ccc
c                    cdstmx1 = max(cdstmx1,dist)
c                    cdstmn1 = min(cdstmn1,dist)
c                    xyzbnd(1,nbnd) = atcrd(1,iat) + srat*(i-atcrd(1,iat))
c                    xyzbnd(2,nbnd) = atcrd(2,iat) + srat*(j-atcrd(2,iat))
c                    xyzbnd(3,nbnd) = atcrd(3,iat) + srat*(k-atcrd(3,iat))
c		      end if
c		    end if
