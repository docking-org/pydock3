...
c
	...
	real*4 srat,ratio,cratmx,cratmn,rratmx,rratmn
...
c	print *,'mapping boundary points to nearest atom/probe center....'
	print *,'mapping boundary points to nearest atom/probe surface....'
	nreen = 0
	nbnd = 0
	ncont = 0
	nbad = 0
...
c nearest atom center
		  ...
		  if(iat.ne.0)then
c
c found nearby atom, scale grid position to be exactly 1 atom rad from it
c
		    ncont = ncont + 1
...
c
c all neighbors of this surface point are in interzone region
c so search linked list for closest girdle point
c
...
c		    print *,'found ',i,j,k,' at ',iat,dmin2
c		    print *,'after searching ',nlook
c
c for closest girdle point 
c scale grid position to be exactly 1 probe rad from it
c
		    if(iat.ne.0)then
		      nreen = nreen + 1
                  dmin2 = (gxyz(1,iat)-i)**2 + (gxyz(2,iat)-j)**2 + (gxyz(3,iat)-k)**2
                  if(dmin2.gt.1.e-6)then
...
cc
                    ...
                    xyzbnd(1,nbnd) = gxyz(1,iat) + srat*(i-gxyz(1,iat))
                    xyzbnd(2,nbnd) = gxyz(2,iat) + srat*(j-gxyz(2,iat))
                    xyzbnd(3,nbnd) = gxyz(3,iat) + srat*(k-gxyz(3,iat))
                  end if
		    else
		      nbad = nbad + 1
c			stop
