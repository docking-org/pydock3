	subroutine qnifft22(natom,atcrd1,atrad1,atcrg1,atfrct,ergt,parfil)
c------------------------------------------------------------------
c version of qnifft that can be called as a subroutine from another program
c requiring solvation forces
c------------------------------------------------------------------
	include 'qdiffpar.h'
c------------------------------------------------------------------
c------------------------------------------------------------------
	...
	logical iatin,iforce
	logical donon,dmmy2
	character*80 radfil,crgfil,pdbfili,pdbfilo,phifili,phifilo
	character*80 epsfil,sitefil,frcfil,sitecrgfil,parfil,pdb2fili
...
c------------------------------------------------------------------
	write(6,*)'   '
	write(6,*)'------------------QNIFFT21--------------------------'
	write(6,*)'* Version 2.1 being called as a subroutine               *'
	write(6,*)'------------------QNIFFT21--------------------------'
	write(6,*)'  '
c------------------------------------------------------------------
...
c
	call getpar(perfil,oldmid,scale,border,isize,epsout,epsin,radprb,
     ...
     &  cion,bfield,iatin,parfil,pdb2fili)
      print *,'cion: ',cion
c
c when we call as subroutine, hard wire some things
	isite = .false.
	iatout = .false.
	iforce = .true.
	ianal = .false.
...
c
	dmmy2 = .false.
c	call getpdb(pdbfili,pdbfilo,iatout,dmmy2,isitecrg,cqplus,cqmin,qplus,qmin,natom,iatin)
	call qcen(natom,atcrd1,atrad1,atcrg1,cqplus,cqmin,qplus,qmin)
c
c calculate scale
c
...
c
c	call mkepsd(natom,exrad,radprb)
...
c
	...
	call phifrc(iforce,donon,rionst,epsin,epsout,natom,iconc,epkt,rtemp,atfrct,ergt)
c
...
c               123456789 123456789 123456789 123456789 123456789 123456789 123456789
	  ...
	  epsin = epsin/epkT
	  epsout = epsout/epkT
	  call phirxn(ianal,epsin,epsout,natom,epkt,rtemp,frcfil)
      end if 
c
