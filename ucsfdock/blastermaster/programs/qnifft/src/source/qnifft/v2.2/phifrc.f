...
c
	...
	real*8 atfrcc(3,0:natom) ! coulombic force
	real*8 atfrcr(3,0:natom) ! reaction field force at atoms
	real*8 atfrcd(3,0:natom) ! stress force at surface
	real*8 atfrci(3,0:natom) ! ionic force
	real*8 atfrct(3,natom) ! total solvent force
	real*4 dr(3),srfx(3),de(3),demag
	real*4 df(3),ftrans(3),frot(3),xcen(3),dr1(3)
	real*4 tin(3,3),froti(3)
	real*4 efact,earea,dpres,phi0
	real*8 ergs,qsurf,ergi,ergt
...
c
	...
	do k = 2,igrid-1
     ...
     &		  + iepsmp(i,j,k,3) + iepsmp(i,j,k-1,3)
		if((isumeps.ne.0).and.(isumeps.ne.6))ibnd = ibnd+1
     ...
     &	  + phimap(i,j,k+1) + phimap(i,j,k-1) + qmap(i,j,k)/epsin)
		   qsurf = qsurf + srfq
c
c use corrected position of surface charge 
c
		  if(fasteps)then
 		    srfx(1) = i
 		    srfx(2) = j
 		    ...
 		    srfx(1) = xyzbnd(1,ibnd)
 		    srfx(2) = xyzbnd(2,ibnd)
 		    srfx(3) = xyzbnd(3,ibnd)
		  end if
c		  print *,'corrected s ijk, xyz,ibnd: ',i,j,k,srfx,ibnd
		  fmagnm = 0.
...
cc using actual atom charge positions
cc
