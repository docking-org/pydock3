...
def format_sphere_line(atom_num, sphere, size=SIZE):
    ...
    return "%5d%10.5f%10.5f%10.5f%8.3f%5d%2d%3d" % (atom_num,
              sphere[0], sphere[1], sphere[2], size,
              atom_num, 0, 0)
...
def main(argv):
    ...
    description = "Generate delphi sphere pool, near a given distance from surface, using rec.ms."
    usage = "%prog [options]"
    ...
    parser.add_option("-d", "--distance", type="float", 
                      help="sphere distance from receptor surface (default: %default)")
    #added by Jiankun Lyu
    parser.add_option("-s", "--size", type="float", 
                      help="sphere radius (default: %default)")
    ...
    return handleio(infile=options.infile, outfile=options.outfile, 
                    distance=options.distance, size=options.size)
