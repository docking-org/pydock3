...
# Make sure the INDOCK file uses the output file sph/match2.sph for docking spheres
# Input files are sph/match.sph, sph/sph from sphgen, and grids/rec.pdb
# Usage: makespheres3.pl 1.0 0.5 60 
...
#    and if too few spheres (<20), repeat continuity with value successively increased by 0.5 until continuity >4.0
#   If too many spheres (>50 for docking): with 25% preference to polar from averageweight
...
#   Print out results to file sph/match2.sph
##########################################################################################
...
#   1 is a crystallographic sphere (should not be deleted ever, until the very end if >50 spheres)
...
#   in eliminating spheres when >50 exist at the end
...
#   $minspheres is arbitrary value of 20 used to keep continuity finding enough spheres, especially open/flat/groove sites
#   $maxspheres is arbitrary value of 50 used near end of script to remove too many spheres for docking
...
$M = 10;              # Margin value to keep spheres close to center of ligand
...
$gridsize = $ARGV[0];      # Typical distance between sphere points, i.e., only one point kept in each grid unit area
$tooclose = $ARGV[1];      # Second pass elimination of spheres that are too close, after nonbiased grid method
$continuity = 3.0;    # How far apart points can be and still considered continuous for docking
...
$minspheres = 20;     # Minimum number of spheres to have for output , niu
$maxspheres = $ARGV[2];     # Maximum number of spheres to have for output, ARBITRARY based on DOCK, niu 
...
$uselig = "on";   # default value, (initialized for later algorithms to check)
...
while ( $line <= $#sphitems ) { 
   if ( $sphitems[$line][1] eq "0" ) {  # starting line of cluster 0 followed by coords
      ...
      print "Starting number of spheres from sph/sph cluster 0: ".$numspherelines."\n";
...
print "FINISHED READING IN FILE sph/sph CLUSTER 0 \n";
...
foreach (keys %polar) {
   ...
   while ($i<=$polarentries{$_}) {
      ...
      #  if((abs($newangle-149.52)<$sphxyzcoords[$_][9])&&($newangle>=116.6508)&&($newangle<=182.3892)) {$sphxyzcoords[$_][7]=$newangle; $sphxyzcoords[$_][9]=abs($newangle-149.52);} # Mean 149.52 Std. Dev. 16.77, fix atom type and etc, niu, 2003
      }
...
if ($#sphxyzcoords+1 > $maxspheres) {       # Arbitrary checkpoint for docking to have no more than 50 spheres
   # Determine center of mass of match.sph
...
#      }
      ...
      for ($i = 0; $i <= $#sphxyzcoords; $i++) { # determine distance of each sphere to center of spheres
       ...
       # setting item 7 can determine how points are weighted when cut off being >50 spheres
...
##################### OUTPUT SPHERES TO FILE sph/match2.sph ##############################
...
# output match2.sph file containing dock coloring header and then sphere coordinates
#$MATCHSPH = "sph/match2.sph";
...
for ($line = 0; $line <= $#sphxyzcoords; $line++ ) {
    ...
    #printf (MATCHSPH "%5.f%10.5f%10.5f%10.5f%8.3f%5.f%2\$d%3\d          \n",(9000+$outline),($sphxyzcoords[$line][1]),($sphxyzcoords[$line][2]),($sphxyzcoords[$line][3]),($sphxyzcoords[$line][4]),($sphxyzcoords[$line][5]),($sphxyzcoords[$line][6]),($sphxyzcoords[$line][7]));
    $outline++;
}
close (MATCHSPH);
