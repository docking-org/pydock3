...
c
      if(ibctyp.eq.5) then
	   print *,'setting external field of (kT/ang): ',bfield
c
c constant field, 1 kT/e/grid unit, in the z direction
c
          do iz=1,igrid
             do iy=1,igrid
                do ix=1,igrid
                   phimap(ix,iy,iz) = bfield*(iz - midg)/scale
	    end do
	    end do
	    end do
	end if
...
c
	open(unit=18,status='old',file=phifili,err=900,form='unformatted')
	write(6,*)' '
...
c
c
c save new grid size, and set temporarily to 65 or whatever max grid dimension
c in qdiffpar.h is
c
	isgrid = igrid
	igrid = ngrid
