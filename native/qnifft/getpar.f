...
c
	dimension oldmid(3)
	logical iper(3),donon,iconc,ibios,isite,isitecrg,iatout,isph,icen,ianal,iatin
	logical fasteps,force
	character*10 sizing,bctyp
	character*60 toplbl
	character*80 radfil,crgfil,pdbfili,pdbfilo,phifili,phifilo
	character*80 epsfil,sitefil,frcfil,sitecrgfil,pdb2fili
	dimension cion(4)
c
...
c
	...
	data names(24,1)  /'field*_boundary      '/
c integers
...
c logicals
	...
	data names(11,3)  /'inpu*t_atm*_file     '/
	data names(12,3)  /'fast*_dielectric_map '/
	data names(13,3)  /'forc*e_calculation   '/
c asci
	...
	data names(14,4)  /'pdb2_in*put_file      '/
c--------------------------------------------------------------
c	call getarg(1,parfil)
	line = parfil
	call elb(line,80)
	call up(line,80)
...
401	continue
	...
	if(ifile.eq.1)then
	  ...
	  print *,'----------------------- '
	  print *,' '
	  open (10,file=deldir(1:il)//'/delphi.def',err=400,status='old')
	else
	  ...
	  print *,'----------------------- '
	  print *,' '
	  open (10,file=parfil,err=400,status='old')
	end if
...
c 
	call up(sizing,10)
	icen = .false.
        if (pdb2fili(1:1) .eq. ' ') then
          hasdef(3,14,4)=.true. !ignore pdb2_in parameter if not present
        end if
...
c if not field, dont need boundary field
	call up(bctyp,10)
...
c
c kas 1-feb-01 correct syntax
c	if((hasdef(1,16,1).eq..false.).and.(hasdef(2,16,1).eq..false.))then
	if((.not.hasdef(1,16,1)).and.(.not.hasdef(2,16,1)))then
	  rionsti = rionst
	  zin = 1.e6
	  hasdef(3,16,1)=.true.
	  hasdef(3,17,1)=.true.
...
c kas 1-feb-01 correct syntax
c	if((hasdef(1,17,1).eq..false.).and.(hasdef(2,17,1).eq..false.))then
	if((.not.hasdef(1,17,1)).and.(.not.hasdef(2,17,1)))then
	  rionsto = rionst
	  zout = 1.e6
	  hasdef(3,18,1)=.true.
	  hasdef(3,19,1)=.true.
...
c
	...
	do while((nnbl.le.len).and.(chr.eq.spc))
	  nnbl = nnbl+1
	  if(nnbl.gt.len)return   ! kas 1-feb-01 fix eol bug
	  chr = line(nnbl:nnbl)
	end do
	return
	end
...
c
	ndlm = ist
	if(ndlm.gt.len)return ! kas 1-feb-01 fix end of line bug
